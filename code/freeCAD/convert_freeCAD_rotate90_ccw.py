...
"""
# NAME:
#   convert_freeCAD_rotate90_ccw.py
#
# PURPOSE:
#   To read in the FreeCAD waveobject formatted file (*.obj) and rotate the geometory 90deg ccw.  
#
# SYNTAX:
...
for i in range(len(nlines)):
    ...
    if (nlines[i][0] == 'v') and (nlines[i][0:2] != 'vt'):
        if (nlines[i][0:2] != 'vn'):
            ...
            nlines[i] = str(ntmp[0])+' '+str(y)+' '+str(x)+' '+str(ntmp[3])+' '+str(ntmp[4])+' '+str(ntmp[5])+\
                    ' '+str(ntmp[6])+'\n'
            print(nlines[i])
